...
const APK="/download/XiaoSuanLife-v1.2.4-debug.apk";
const tools=[
...
function Nav(){const[open,setOpen]=useState(false),[scroll,setScroll]=useState(false);useEffect(()=>{const f=()=>setScroll(window.scrollY>20);f();addEventListener("scroll",f);return()=>removeEventListener("scroll",f)},[]);return <header className={scroll?"nav scrolled":"nav"}><div className="nav-inner"><Logo/><nav className={open?"open":""}><a href="#tools" onClick={()=>setOpen(false)}>全部工具</a><a href="#screens" onClick={()=>setOpen(false)}>真实界面</a><a href="#system" onClick={()=>setOpen(false)}>产品能力</a><a href="#about" onClick={()=>setOpen(false)}>关于</a><a className="nav-cta" href={APK} download><Download/>下载 v1.2.4</a></nav><button className="menu" onClick={()=>setOpen(!open)} aria-label="菜单">{open?<X/>:<Menu/>}</button></div></header>}

function Phone(){return <div className="phone-shell"><div className="phone-screen"><img src="/screens/home.jpg" alt="小算生活工具首页"/></div></div>}

export default function Home(){const[active,setActive]=useState(0);const prev=()=>setActive((active-1+gallery.length)%gallery.length),next=()=>setActive((active+1)%gallery.length);const openTool=(shot:string)=>{const index=gallery.findIndex(([id])=>id===shot);if(index>=0)setActive(index);requestAnimationFrame(()=>document.getElementById("screens")?.scrollIntoView({behavior:"smooth",block:"start"}))};return <main><Nav/>
 <section className="hero"><div className="hero-grain"/><div className="hero-inner"><motion.div className="hero-copy" initial={{opacity:0,y:25}} animate={{opacity:1,y:0}} transition={{duration:.65}}><div className="version-pill"><span/> Android v1.2.4 已开放下载</div><h1>把生活里的计算，<br/><em>一次装进口袋。</em></h1><p>从基础计算、房贷个税，到健康管理与亲戚称呼。小算生活把 11+ 种常用工具做得清晰、轻巧、随手可用。</p><div className="hero-actions"><a href={APK} download className="primary"><Download/>下载 Android APK <small>17.27 MB</small></a><a href="#screens" className="secondary">查看真实界面 <ArrowRight/></a></div><div className="hero-proof"><span><b>11+</b>计算工具</span><span><b>31</b>历史记录</span><span><b>100%</b>本地存储</span></div></motion.div><motion.div className="hero-product" initial={{opacity:0,scale:.93,x:20}} animate={{opacity:1,scale:1,x:0}} transition={{duration:.75}}><div className="phone-halo"/><Phone/><div className="floating-card one"><Landmark/><span><b>房贷计算</b>月供 ¥4,490.45</span></div><div className="floating-card two"><HeartPulse/><span><b>BMI 22.49</b>健康状态正常</span></div></motion.div></div></section>
 ...
 <section className="numbers"><div><span><b>11+</b>实用计算工具</span><span><b>165+</b>汇率币种信息</span><span><b>337</b>地级行政区选择</span><span><b>0</b>云端隐私上传</span></div></section>

 <section className="download-cta"><div><span className="label light">DOWNLOAD NOW</span><h2>现在，开始你的小算生活。</h2><p>小算生活 XiaoSuanLife v1.2.4 · 支持 Android 8.0+</p></div><div><a href={APK} download className="white-button"><Download/>直接下载 APK <ArrowRight/></a><Link href="/download">查看二维码与版本校验</Link></div></section>
